...
// create user
export const userRegister = (data,e,setInput,setRegister,navigate) => async (dispatch) => {
    try {
...
export const accountActivationByCode = ({ code, auth }, navigate) => async (dispatch) => {
    try {
       
       await axios.post("/api/v1/user/code-activation", {
            code : code,
            auth : auth
        })
        .then(res => {
...
export const resendActivationLink = ( email , navigate) => async (dispatch) => {
    try {
        ...
        await axios.post("/api/v1/user/resend-activation", {
            auth : email
        })
        .then(res => {
            createToast(res.data.message , "success")
...
export const resendResetPasswordCode= ( auth ) => async (dispatch) => {
    ...
    try {
        ...
        await axios.post("/api/v1/user/resend-reset-otp", {
            auth : auth
        })
        .then(res => {
            createToast(res.data.message , "success")
 ...
 */
export const checkResetOTP = ({ code, auth }, navigate) => async (dispatch) => {
    try {
       
       await axios.post("/api/v1/user/password-reset-otp-check", {
            code : code,
            auth : auth
        })
        .then(res => {
 ...
 */
export const updateUserInfo = (data, id, setCatShow, setJobShow, setEduShow, setUniShow, setCityShow, setTownShow) => async (dispatch) => {
