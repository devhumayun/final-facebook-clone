    ...
    if (user) {
      if (email_data) {
        ...
        sendActivationLink(user.email, {
          ...
          }/api/v1/user/account-activate/${activation_token}`,
...
export const resendActivationLink = async (req, res, next) => {
  ...
  try {
    ...
    if (mobile_data) {
      ...
      res
        .status(200)
        .cookie('otp', mobile_check.mobile, {
          expires: new Date(Date.now() + 1000 * 60 * 15),
        })
        .json({
  ...
  try {
    ...
    if (user) {
      ...
      sendpasswordResetLink(user.email, {
        ...
        link: `${
          process.env.APP_URL + ':' + process.env.SERVER_PORT
        }/api/v1/user/forget-password/${passwordReset_token}`,
        code: activation_code,
      })
